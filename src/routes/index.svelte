<script>
	...
	function submitHandler() {
		let newGear = new Gear(type, name, augments);
...
<div class="flex items-center justify-center text-center">
	<Box>
		<h1 class="py-4 text-3xl font-bold">Create New Gear</h1>
		<form class="flex flex-col gap-6 rounded-lg border p-6">
			<div class="flex flex-col items-start">
				<label>
					Unit
					<input required value="unit" type="radio" bind:group={type} />
				</label>
				<label>
					Weapon
					<input required value="weapon" type="radio" bind:group={type} />
				</label>
			</div>
